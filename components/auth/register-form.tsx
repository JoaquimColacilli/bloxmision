...
export function RegisterForm({ onSubmit, onGoogleAuth, onSwitchToLogin, isLoading }: RegisterFormProps) {
  ...
  const validateDisplayName = (value: string): string | undefined => {
    if (!value.trim()) return "¿Cómo te llamamos, marinero?"
    if (value.trim().length < 2) return "Tu nombre de pirata necesita al menos 2 letras"
    return undefined
  }
  ...
  const validatePassword = (value: string): string | undefined => {
    ...
    if (value.length < 6) return "Tu contraseña secreta necesita al menos 6 letras"
    return undefined
  }

  const handleBlur = (field: "displayName" | "email" | "password") => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <AuthInput
        label="Nombre de pirata"
        type="text"
        placeholder="Capitán Barbanegra"
        value={displayName}
